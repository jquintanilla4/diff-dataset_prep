...
one_shot_example = [{
    "role": "user",
    "parts": [prompt, {"inline_data": {"data": sample_file_1_jpeg, "mime_type": "image/jpeg"}}]
}, {
    "role": "model",
    "parts": [
...
def process_folder(folder_path, model, one_shot_example, prompt, max_images=3000):
    """Process up to max_images images in the given folder, handling them in batches."""
    ...
    with tqdm(total=len(image_files), desc="Processing images") as pbar:
        batch_size = 10  # Define the batch size here
        for i in range(0, len(image_files), batch_size):
            batch_files = image_files[i:i + batch_size]
            messages = []  # Initialize messages list for the current batch
            for filename in batch_files:
                ...
                try:
                    ...
                    new_image_message = {
                        "role": "user",
                        "parts": [prompt, {"inline_data": {"data": new_image_jpeg, "mime_type": "image/jpeg"}}]
                    }
                    messages.append(new_image_message)
                except Exception as e:
                    print(f"Error preparing {filename}: {e}")
                    pbar.update(1)

            # Add the one-shot example only to the first message in the batch
            messages_with_example = one_shot_example + messages

            # Generate content with retry logic for the entire batch
            try:
                responses = generate_with_retry(model, messages_with_example)

                # Process responses for the current batch
                for j, response in enumerate(responses):
                    txt_filename = os.path.splitext(batch_files[j])[0] + ".txt"
                    txt_path = os.path.join(folder_path, txt_filename)
                    with open(txt_path, 'w', encoding='utf-8') as f:
                        f.write(response.text)
                    print(f"Processed {batch_files[j]} -> {txt_path}")
                    print(f"Response:\n{response.text}\n")
            except Exception as e:
